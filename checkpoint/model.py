...
model.py
--------
Lightweight UNet that maps an input image to a bounded perturbation.

Architecture overview
---------------------
    Input (3, H, W)
        │
    Encoder: 3 downsampling blocks (Conv + GroupNorm + ReLU)
        │
    Bottleneck: 2 conv layers
        │
    Decoder: 3 upsampling blocks (Upsample + Conv + GroupNorm + ReLU)
             each receives skip connection from the matching encoder block
        │
    Head: 1×1 Conv → Tanh → scale by epsilon
        │
    Output perturbation (3, H, W)  ∈ [-epsilon, +epsilon]

Design decisions for CPU training
----------------------------------
- base_channels=8 keeps parameter count tiny (~50k params vs 31M for UNet).
- GroupNorm(num_groups=4) works with batch size 1 (BatchNorm needs >1).
- Tanh output automatically enforces the L-inf epsilon bound.
- No dropout (not needed; the adversarial objective acts as regularisation).
"""
...
def _down(in_ch: int, out_ch: int) -> tuple[nn.Sequential, nn.MaxPool2d]:
    """Encoder step: conv block + 2× max-pool."""
    return _conv_block(in_ch, out_ch), nn.MaxPool2d(2)


def _up(in_ch: int, out_ch: int) -> nn.Sequential:
    """Decoder step: 2× bilinear upsample + conv block."""
    # Note: forward() concatenates the skip before calling this,
    # so in_ch already accounts for the doubled channel count.
...
class PerturbationUNet(nn.Module):
    """
    Image → bounded adversarial perturbation.

    Parameters
    ----------
    base_channels : int
        Number of feature maps in the first encoder block.
        All subsequent blocks multiply this by powers of 2.
    epsilon : float
        L-inf bound on the output perturbation.
    """
    ...
    def __init__(self, base_channels: int = 8, epsilon: float = 8 / 255.0):
        ...
        # ---- Decoder ------------------------------------------------ #
        # After upsampling, skip is concatenated → channel count doubles.
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        ----------
        x : torch.Tensor
            Input image tensor, shape (B, 3, H, W), values in [0, 1].

        Returns
        -------
        adv_image : torch.Tensor
            Adversarial image clamped to [0, 1], same shape as x.
        perturbation : torch.Tensor
            Raw perturbation in [-epsilon, +epsilon], same shape as x.
        """
        # Encoder
        ...
        raw = self.head(d1)
        perturbation = self.epsilon * torch.tanh(raw)          # ∈ [-ε, +ε]
...
def build_perturbation_net(config: Config = cfg) -> PerturbationUNet:
    """Instantiate the perturbation network from a Config object."""
